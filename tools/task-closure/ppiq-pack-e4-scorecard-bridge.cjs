...
const bridgedJsonPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_E4_BRIDGED.json");
const bridgedMdPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_E4_BRIDGED.md");
...
function setDone(row, note) {
  row.score = 100; row.percent = 100; row.completionPercent = 100; row.percentage = 100;
  row.status = "DONE"; row.state = "DONE"; row.result = "DONE";
  row.isGreen = true; row.isDone = true; row.done = true; row.below90 = false;
...
const t064Green = runOk("node", ["tools/pack-e/validate-pack-e-t064-historian-regression.cjs"]);

for (const row of rows) {
  if (code(row) === "T-064" && t064Green) setDone(row, "Pack E-4 evidence bridge: historian tests/docs/regression validator passed.");
}

scorecard.packE4EvidenceBridge = { generatedAtUtc: new Date().toISOString(), marker: "PPIQ_PACK_E4_T064_SCORECARD_BRIDGE", t064Green };
...
const md = [];
md.push("# T001-T071 Task Closure Scorecard — Pack E4 Bridged");
md.push("");
md.push("Marker: PPIQ_PACK_E4_T064_SCORECARD_BRIDGE");
md.push("");
md.push("T-064 bridge result: " + (t064Green ? "DONE" : "NOT GREEN"));
md.push("");
md.push("Tasks below 90% after Pack E4 bridge: " + below90.length);
md.push("");
...
console.log("");
console.log("Pack E4 task-closure evidence bridge applied.");
console.log("T-064 bridge result: " + (t064Green ? "DONE" : "NOT GREEN"));
console.log("");
console.log("Tasks below 90% after Pack E4 bridge: " + below90.length);
