...
const bridgedJsonPath = path.join(root, "docs", "task-closure", "T096_T102_PHASE15_SCORECARD.PACK_G6_BRIDGED.json");
const bridgedMdPath = path.join(root, "docs", "task-closure", "T096_T102_PHASE15_SCORECARD.PACK_G6_BRIDGED.md");
...
const t099Green = runOk("node", ["tools/pack-g/validate-pack-g6-t099-roi-cfo-dashboard.cjs"]);

for (const row of rows) {
  if (code(row) === "T-099" && t099Green) setDone(row, "Pack G-6 evidence bridge: ROI/CFO value dashboard validator passed and builds were green.");
}

scorecard.packG6EvidenceBridge = { generatedAtUtc: new Date().toISOString(), marker: "PPIQ_PACK_G6_T099_SCORECARD_BRIDGE", t099Green };
...
const md = [];
md.push("# T096-T102 Phase 15 Scorecard — Pack G6 Bridged");
md.push("");
md.push("Marker: PPIQ_PACK_G6_T099_SCORECARD_BRIDGE");
md.push("");
md.push("T-099 bridge result: " + (t099Green ? "DONE" : "NOT GREEN"));
md.push("");
md.push("Tasks below 90% after Pack G6 bridge: " + below90.length);
md.push("");
...
console.log("");
console.log("Pack G6 task-closure evidence bridge applied.");
console.log("T-099 bridge result: " + (t099Green ? "DONE" : "NOT GREEN"));
console.log("");
console.log("Tasks below 90% after Pack G6 bridge: " + below90.length);
