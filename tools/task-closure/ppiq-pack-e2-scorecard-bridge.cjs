...
const bridgedJsonPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_E2_BRIDGED.json");
const bridgedMdPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_E2_BRIDGED.md");
...
const t060Green = runOk("node", ["tools/pack-e/validate-pack-e-t060-ga-historian-backend.cjs"]);

for (const row of rows) {
  if (code(row) === "T-060" && t060Green) setDone(row, "Pack E-2 evidence bridge: GA historian backend validator passed and backend build was green.");
}

scorecard.packE2EvidenceBridge = { generatedAtUtc: new Date().toISOString(), marker: "PPIQ_PACK_E2_T060_SCORECARD_BRIDGE", t060Green };
...
const md = [];
md.push("# T001-T071 Task Closure Scorecard — Pack E2 Bridged");
md.push("");
md.push("Marker: PPIQ_PACK_E2_T060_SCORECARD_BRIDGE");
md.push("");
md.push("T-060 bridge result: " + (t060Green ? "DONE" : "NOT GREEN"));
md.push("");
md.push("Tasks below 90% after Pack E2 bridge: " + below90.length);
md.push("");
...
console.log("");
console.log("Pack E2 task-closure evidence bridge applied.");
console.log("T-060 bridge result: " + (t060Green ? "DONE" : "NOT GREEN"));
console.log("");
console.log("Tasks below 90% after Pack E2 bridge: " + below90.length);
