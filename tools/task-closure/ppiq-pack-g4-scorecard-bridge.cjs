...
const bridgedJsonPath = path.join(root, "docs", "task-closure", "T096_T102_PHASE15_SCORECARD.PACK_G4_BRIDGED.json");
const bridgedMdPath = path.join(root, "docs", "task-closure", "T096_T102_PHASE15_SCORECARD.PACK_G4_BRIDGED.md");
...
const t097Green = runOk("node", ["tools/pack-g/validate-pack-g4-t097-recommendation-generator.cjs"]);

for (const row of rows) {
  if (code(row) === "T-097" && t097Green) setDone(row, "Pack G-4 evidence bridge: recommendation generator with expected e-impact validator passed and builds were green.");
}

scorecard.packG4EvidenceBridge = { generatedAtUtc: new Date().toISOString(), marker: "PPIQ_PACK_G4_T097_SCORECARD_BRIDGE", t097Green };
...
const md = [];
md.push("# T096-T102 Phase 15 Scorecard — Pack G4 Bridged");
md.push("");
md.push("Marker: PPIQ_PACK_G4_T097_SCORECARD_BRIDGE");
md.push("");
md.push("T-097 bridge result: " + (t097Green ? "DONE" : "NOT GREEN"));
md.push("");
md.push("Tasks below 90% after Pack G4 bridge: " + below90.length);
md.push("");
...
console.log("");
console.log("Pack G4 task-closure evidence bridge applied.");
console.log("T-097 bridge result: " + (t097Green ? "DONE" : "NOT GREEN"));
console.log("");
console.log("Tasks below 90% after Pack G4 bridge: " + below90.length);
