...
const bridgedJsonPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_F5_BRIDGED.json");
const bridgedMdPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_F5_BRIDGED.md");
...
const t071Green = runOk("node", ["tools/pack-f/validate-pack-f-t071-edge-regression.cjs"]);

for (const row of rows) {
  if (code(row) === "T-071" && t071Green) setDone(row, "Pack F-5 evidence bridge: edge tests/docs/regression validator passed; backend and frontend builds were green.");
}

scorecard.packF5EvidenceBridge = { generatedAtUtc: new Date().toISOString(), marker: "PPIQ_PACK_F5_T071_SCORECARD_BRIDGE", t071Green };
...
const md = [];
md.push("# T001-T071 Task Closure Scorecard — Pack F5 Bridged");
md.push("");
md.push("Marker: PPIQ_PACK_F5_T071_SCORECARD_BRIDGE");
md.push("");
md.push("T-071 bridge result: " + (t071Green ? "DONE" : "NOT GREEN"));
md.push("");
md.push("Tasks below 90% after Pack F5 bridge: " + below90.length);
md.push("");
...
console.log("");
console.log("Pack F5 task-closure evidence bridge applied.");
console.log("T-071 bridge result: " + (t071Green ? "DONE" : "NOT GREEN"));
console.log("");
console.log("Tasks below 90% after Pack F5 bridge: " + below90.length);
