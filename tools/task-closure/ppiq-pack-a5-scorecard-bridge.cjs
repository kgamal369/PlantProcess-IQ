...
const bridgedJsonPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_A5_BRIDGED.json");
const bridgedMdPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_A5_BRIDGED.md");
...
const t035Green = runOk("node", ["tools/pack-a/validate-pack-a-t035-mapping-drift-docs.cjs"]);

for (const row of rows) {
  if (code(row) === "T-035" && t035Green) setDone(row, "Pack A-5 evidence bridge: mapping and drift developer docs validator passed.");
}

scorecard.packA5EvidenceBridge = { generatedAtUtc: new Date().toISOString(), marker: "PPIQ_PACK_A5_T035_SCORECARD_BRIDGE", t035Green };
...
const md = [];
md.push("# T001-T071 Task Closure Scorecard — Pack A5 Bridged");
md.push("");
md.push("Marker: PPIQ_PACK_A5_T035_SCORECARD_BRIDGE");
md.push("");
md.push("T-035 bridge result: " + (t035Green ? "DONE" : "NOT GREEN"));
md.push("");
md.push("Tasks below 90% after Pack A5 bridge: " + below90.length);
md.push("");
...
console.log("");
console.log("Pack A5 task-closure evidence bridge applied.");
console.log("T-035 bridge result: " + (t035Green ? "DONE" : "NOT GREEN"));
console.log("");
console.log("Tasks below 90% after Pack A5 bridge: " + below90.length);
