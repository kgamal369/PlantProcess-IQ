...
const bridgedJsonPath = path.join(root, "docs", "task-closure", "T096_T102_PHASE15_SCORECARD.PACK_G8_BRIDGED.json");
const bridgedMdPath = path.join(root, "docs", "task-closure", "T096_T102_PHASE15_SCORECARD.PACK_G8_BRIDGED.md");
...
const t101Green = runOk("node", ["tools/pack-g/validate-pack-g8-t101-honesty-certification.cjs"]);

for (const row of rows) {
  if (code(row) === "T-101" && t101Green) setDone(row, "Pack G-8 evidence bridge: recommendation honesty and approval certification validator passed and builds were green.");
}

scorecard.packG8EvidenceBridge = { generatedAtUtc: new Date().toISOString(), marker: "PPIQ_PACK_G8_T101_SCORECARD_BRIDGE", t101Green };
...
const md = [];
md.push("# T096-T102 Phase 15 Scorecard — Pack G8 Bridged");
md.push("");
md.push("Marker: PPIQ_PACK_G8_T101_SCORECARD_BRIDGE");
md.push("");
md.push("T-101 bridge result: " + (t101Green ? "DONE" : "NOT GREEN"));
md.push("");
md.push("Tasks below 90% after Pack G8 bridge: " + below90.length);
md.push("");
...
console.log("");
console.log("Pack G8 task-closure evidence bridge applied.");
console.log("T-101 bridge result: " + (t101Green ? "DONE" : "NOT GREEN"));
console.log("");
console.log("Tasks below 90% after Pack G8 bridge: " + below90.length);
