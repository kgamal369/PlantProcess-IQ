...
const bridgedJsonPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_F3_BRIDGED.json");
const bridgedMdPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_F3_BRIDGED.md");
...
const t067Green = runOk("node", ["tools/pack-f/validate-pack-f-t067-edge-packaging.cjs"]);

for (const row of rows) {
  if (code(row) === "T-067" && t067Green) setDone(row, "Pack F-3 evidence bridge: edge packaging/deployment validator passed.");
}

scorecard.packF3EvidenceBridge = { generatedAtUtc: new Date().toISOString(), marker: "PPIQ_PACK_F3_T067_SCORECARD_BRIDGE", t067Green };
...
const md = [];
md.push("# T001-T071 Task Closure Scorecard — Pack F3 Bridged");
md.push("");
md.push("Marker: PPIQ_PACK_F3_T067_SCORECARD_BRIDGE");
md.push("");
md.push("T-067 bridge result: " + (t067Green ? "DONE" : "NOT GREEN"));
md.push("");
md.push("Tasks below 90% after Pack F3 bridge: " + below90.length);
md.push("");
...
console.log("");
console.log("Pack F3 task-closure evidence bridge applied.");
console.log("T-067 bridge result: " + (t067Green ? "DONE" : "NOT GREEN"));
console.log("");
console.log("Tasks below 90% after Pack F3 bridge: " + below90.length);
