...
const bridgedJsonPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_A4_BRIDGED.json");
const bridgedMdPath = path.join(root, "docs", "task-closure", "T001_T071_TASK_CLOSURE_SCORECARD.PACK_A4_BRIDGED.md");
...
const t007Green = runOk("node", ["tools/pack-a/validate-pack-a-t007-dedup.cjs"]);

for (const row of rows) {
  if (code(row) === "T-007" && t007Green) setDone(row, "Pack A-4 evidence bridge: T-007 dedup validator passed and backend build was green.");
}

scorecard.packA4EvidenceBridge = { generatedAtUtc: new Date().toISOString(), marker: "PPIQ_PACK_A4_T007_SCORECARD_BRIDGE", t007Green };
...
const md = [];
md.push("# T001-T071 Task Closure Scorecard — Pack A4 Bridged");
md.push("");
md.push("Marker: PPIQ_PACK_A4_T007_SCORECARD_BRIDGE");
md.push("");
md.push("T-007 bridge result: " + (t007Green ? "DONE" : "NOT GREEN"));
md.push("");
md.push("Tasks below 90% after Pack A4 bridge: " + below90.length);
md.push("");
...
console.log("");
console.log("Pack A4 task-closure evidence bridge applied.");
console.log("T-007 bridge result: " + (t007Green ? "DONE" : "NOT GREEN"));
console.log("");
console.log("Tasks below 90% after Pack A4 bridge: " + below90.length);
