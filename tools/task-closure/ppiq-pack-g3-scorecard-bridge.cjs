...
const bridgedJsonPath = path.join(root, "docs", "task-closure", "T096_T102_PHASE15_SCORECARD.PACK_G3_BRIDGED.json");
const bridgedMdPath = path.join(root, "docs", "task-closure", "T096_T102_PHASE15_SCORECARD.PACK_G3_BRIDGED.md");
...
const t096Green = runOk("node", ["tools/pack-g/validate-pack-g3-t096-scenario-engine.cjs"]);

for (const row of rows) {
  if (code(row) === "T-096" && t096Green) setDone(row, "Pack G-3 evidence bridge: deterministic what-if scenario engine validator passed and builds were green.");
}

scorecard.packG3EvidenceBridge = { generatedAtUtc: new Date().toISOString(), marker: "PPIQ_PACK_G3_T096_SCORECARD_BRIDGE", t096Green };
...
const md = [];
md.push("# T096-T102 Phase 15 Scorecard — Pack G3 Bridged");
md.push("");
md.push("Marker: PPIQ_PACK_G3_T096_SCORECARD_BRIDGE");
md.push("");
md.push("T-096 bridge result: " + (t096Green ? "DONE" : "NOT GREEN"));
md.push("");
md.push("Tasks below 90% after Pack G3 bridge: " + below90.length);
md.push("");
...
console.log("");
console.log("Pack G3 task-closure evidence bridge applied.");
console.log("T-096 bridge result: " + (t096Green ? "DONE" : "NOT GREEN"));
console.log("");
console.log("Tasks below 90% after Pack G3 bridge: " + below90.length);
