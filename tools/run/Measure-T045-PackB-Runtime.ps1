...
function Show-Result($outcome, [string]$label) {
    W ''
    W ('  --- ' + $label + ' ---')
    if (-not $outcome.Ok) {
        W ('      REFUSED or FAILED')
...
$fsR = Show-Result $fs 'findingStatus (correlation_results)'
if ($fsR -ne $null) {
    $state = $null
...
$scR = Show-Result $sc 'scoringCoverage (risk_scores + material_units)'
if ($scR -ne $null) {
...
$arR = Show-Result $ar ('analysisReadiness (parameterCode = ' + $(if ($ChosenOutcome) { $ChosenOutcome } else { '(none)' }) + ')')
if ($arR -ne $null) {
    $overall = $null
...
Show-Result $ar0 'analysisReadiness with no target (the empty-install state)' | Out-Null
...
$c1R = Show-Result $c1 'CONTROL materialCount by materialUnitType (Class 1, migrated)'
if ($c1R -ne $null) {
    $cols = @()
...
$d1R = Show-Result $d1 'D1 REGRESSION observationCount (exact, must never equal the 50000 cap)'
if ($d1R -ne $null) {
    $total = 0
...
Show-Result $c1blocked 'CONTROL processStepDuration (unmigrated, expected containment refusal)' | Out-Null
