...
function Sql([string]$Query) {
    $out = & psql -h 127.0.0.1 -p 5432 -U $PgUser -d $PgDb -w -X -A -t -F '|' -v ON_ERROR_STOP=1 -c $Query 2>&1
...
SELECT parameter_code, parameter_name, unit_of_measure,
       coalesce(expected_min_value::text,''), coalesce(expected_max_value::text,'')
...
if ($approved.Count -eq 0) {
    ...
    $bits = $approved[0].ToString().Split('|')
    $code = $bits[0]; $name = $bits[1]; $unit = $bits[2]; $min = $bits[3]; $max = $bits[4]
    Note ("chosen: " + $code + " (" + $name + ")  unit=" + $unit + "  min=" + $min + "  max=" + $max)

    $secondQuestion = "what is the " + $name.ToLowerInvariant()
    Note ("question built from the row's own name: " + $secondQuestion)

    $second = $null
    try { $second = Ask $secondQuestion } catch { }
