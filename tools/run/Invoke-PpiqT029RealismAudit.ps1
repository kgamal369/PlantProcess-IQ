...
Rule "PPIQ T-029 - FIVE-LAYER REALISM AUDIT"
...
SELECT 'coils with no genealogy parent'                 || '~' || count(*)::text FROM public.material_units mu
  WHERE mu.is_deleted = false AND lower(coalesce(mu.material_unit_type,'')) LIKE '%coil%'
    AND NOT EXISTS (SELECT 1 FROM public.genealogy_edges ge WHERE ge.child_unit_id = mu.id)
UNION ALL
...
SELECT 'genealogy edges with a dangling parent'         || '~' || count(*)::text FROM public.genealogy_edges ge
  LEFT JOIN public.material_units p ON p.id = ge.parent_unit_id WHERE p.id IS NULL
UNION ALL
SELECT 'genealogy edges with a dangling child'          || '~' || count(*)::text FROM public.genealogy_edges ge
  LEFT JOIN public.material_units c ON c.id = ge.child_unit_id WHERE c.id IS NULL
UNION ALL
...
JOIN public.material_units p ON p.id = ge.parent_unit_id AND p.is_deleted = false
JOIN public.material_units c ON c.id = ge.child_unit_id AND c.is_deleted = false
...
BEGIN
  BEGIN
    INSERT INTO public.genealogy_edges (id, parent_unit_id, child_unit_id)
...
`$t`$;
ROLLBACK;
SELECT 'rows left behind~' || count(*)::text FROM public.genealogy_edges
WHERE parent_unit_id NOT IN (SELECT id FROM public.material_units);
"@
        Say $r.Output
        Say ($r.Error.Trim())
    }
...
$ev = Join-Path $outFolder ("T-029_realism_audit_" + $stamp + ".txt")
