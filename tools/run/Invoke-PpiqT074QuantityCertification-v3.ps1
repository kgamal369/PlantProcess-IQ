...
# The optional section cannot decide certification, so its problems are WARN.
# A tooling defect there must never mark a passing mandatory check as failed.
function Warn([string]$Text) { W ("WARN  " + $Text) }
...
# Two earlier runs returned a one-character row here. Both used a psql command
# line containing a literal pipe - once in -F, once in the separator - and every
# query without one worked. No pipe reaches psql from this version.
Note "separator: three tildes, chosen because a pipe breaks the native command line"
...
if ($approved.Count -eq 0) {
    ...
    $bits = $raw -split '~~~'
    if ($bits.Count -lt 5) {
        Warn ("the approved row did not parse into five fields; skipping the optional check")
        $bits = $null
    }
}
...
if ($approved.Count -gt 0 -and $null -ne $bits -and $bits.Count -ge 5) {
    ...
    if ([string]::IsNullOrWhiteSpace($unit) -or [string]::IsNullOrWhiteSpace($name)) {
        Warn "the chosen row has no usable unit or name; skipping rather than checking every number"
        $name = $null
    }
    ...
    if ($null -eq $secondQuestion) {
        ...
        Warn "the approved-definition ask failed"
    } else {
...
W ""
W "The MANDATORY casting-speed check is what certifies T-074. The optional"
W "approved-definition check is reported and never decides the outcome."
W ""
if ($Script:Fail -eq 0) {
