...
.DESCRIPTION
    ...
    THE 26 JULY RESULTS DO NOT COUNT. The gate measures the CURRENT result
    population separately from the historical one and requires the current one to
    exceed zero. This script does NOT delete the historical rows by default -
    removing analysis history was not authorised. -QuarantineStaleResults is
    provided so that decision is explicit and visible, never a side effect.
...
.EXAMPLE
    .\tools\run\Invoke-PpiqT025Corrective.ps1
    .\tools\run\Invoke-PpiqT025Corrective.ps1 -QuarantineStaleResults
#>
...
$Marker      = "PPIQ T-025b outcome producer correction"
...
Rule "PPIQ T-025 CORRECTIVE EXECUTION"
...
WHERE n.nspname = 'public' AND p.proname = 'ppiq_ml_refresh_feature_store'
  AND p.prokind = 'f'
  AND pg_get_functiondef(p.oid) LIKE '%$Marker%';
"@
    Say ("  functions carrying the T-025b marker : " + $marked + " (required 1)")
    if ($marked -ne "1") {
        Say "[STOP] the corrected producer is not installed. Apply T-025b v2 first."
...
"@
    Say $split.Output
    $curResults = Get-Scalar -Tag "curresults" -Sql @"
...
"@
    $histResults = Get-Scalar -Tag "histresults" -Sql @"
SELECT count(r.id)::text
FROM public.ml_correlation_results_v2 r
JOIN public.ml_correlation_compute_runs c ON c.id = r.compute_run_id
WHERE c.completed_at_utc < TIMESTAMPTZ '$RunStartUtc+00';
"@
    Say ""
    Say ("  CURRENT Fleet v2 result population : " + $curResults + " (required above 0)")
    Say ("  historical result population       : " + $histResults)
    if ([int]$curResults -le 0) {
        $script:Gate["Current correlation population"] = "FAIL - zero current results"
        $bad = $bad + 1
    } else {
        $script:Gate["Current correlation population"] = ("PASS - " + $curResults + " current")
    }

    if ($QuarantineStaleResults) {
        Say ""
        Say "  -QuarantineStaleResults WAS PASSED. Deleting result rows whose compute"
        Say "  run predates this execution. Run metadata is preserved as history."
...
"@
        ...
        Say ""
        Say "  NOT QUARANTINED. Removing analysis history was not authorised, so this"
        Say "  script reports the split and leaves the rows in place. Re-run with"
        Say "  -QuarantineStaleResults if the historical rows must be removed before"
        Say "  closure rather than filtered by compute run on the reading surfaces."
    }
...
foreach ($k in @("Authenticated product path", "Outcome populations",
                 "Current correlation population", "Risk", "Learning",
...
$ev = Join-Path $outFolder ("T-025_corrective_" + $stamp + ".txt")
