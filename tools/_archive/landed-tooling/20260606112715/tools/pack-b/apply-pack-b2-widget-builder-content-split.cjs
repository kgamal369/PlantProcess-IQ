...
const backupRoot = path.join(root, ".pack_b_backup", "pack_b2_widget_content_" + timestamp);
...
function splitMainComponent(mainBlock) {
  ...
  const signatureEnd = lines.findIndex((line) => line.includes("}) {"));
  if (signatureEnd < 0) {
  ...
  for (let i = signatureEnd + 1; i < lines.length; i += 1) {
    ...
    if (beforeDepth === 1 && /^\s*return\s*\(/.test(line)) {
      returnStart = i;
      break;
    }
    ...
    if (i === signatureEnd + 1 && depth === 0) {
      depth = 1;
    }
  }

  if (returnStart < 0) {
    // fallback: find last top-level-ish return
    returnStart = lines.findIndex((line) => /^\s*return\s*\(/.test(line));
  }

  if (returnStart < 0) {
    throw new Error("Could not locate main return block.");
  }

  const functionHeader = lines.slice(0, signatureEnd + 1).join("\n");
  const preReturn = lines.slice(signatureEnd + 1, returnStart).join("\n").replace(/^\s*\{\s*\n?/, "");
...
function convertRelativeImports(importBlock, fromNewFileDepth) {
  // New files are under ./content, one level deeper than the original widget-builder folder.
  return importBlock
...
function main() {
  console.log("=================================================================================================");
  console.log("PlantProcess IQ Pack B-2 — WidgetBuilderWizardContent semantic split");
  console.log("=================================================================================================");
  ...
  function stepModule(fileName, exportNames, extraImports) {
    const parts = [];
    ...
    write(
      ...
      [
        ...
        "import { selectFieldForDimension, mapValidationIssues } from \"./WidgetBuilderWizardContent.helpers\";",
        extraImports || "",
        "",
        wizardSection,
        "",
  ...
  const report = {
    ...
    marker: "PPIQ_PACK_B2_WIDGET_BUILDER_CONTENT_SPLIT",
  ...
  write(path.join(root, "docs", "pack-b", "PACK_B2_WIDGET_CONTENT_SPLIT_REPORT.json"), JSON.stringify(report, null, 2) + "\n");

  let evidence = isFile(evidencePath)
    ? normalize(read(evidencePath))
    : "# PlantProcess IQ Pack B Evidence\n";

  if (!evidence.includes("PPIQ_PACK_B2_WIDGET_BUILDER_CONTENT_SPLIT")) {
    evidence += [
      "",
      "## Pack B-2 WidgetBuilderWizardContent split",
      "",
      "- Marker: `PPIQ_PACK_B2_WIDGET_BUILDER_CONTENT_SPLIT`.",
      "- Replaced `WidgetBuilderWizardContent.implementation.tsx` with a thin wrapper.",
      "- Extracted types, helpers, data/chart/purpose steps, filter step, script/preview steps, model hook, view, and orchestrator.",
  ...
  run("node --check content orchestrator", ["node", "--check", "Frontend/PlantProcess.Web/src/components/dashboard/widget-builder/content/WidgetBuilderWizardContent.orchestrator.tsx"], root, true);

  run("Frontend npm build", npmArgs("run", "build"), frontendRoot);

  if (isFile(path.join(root, "tools", "phase56", "validate-phase56.cjs"))) {
    run("Phase 5/6 validation", ["node", "tools/phase56/validate-phase56.cjs"], root);
  }
  ...
  console.log("");
  console.log("=================================================================================================");
  console.log("Pack B-2 WidgetBuilderWizardContent split completed.");
