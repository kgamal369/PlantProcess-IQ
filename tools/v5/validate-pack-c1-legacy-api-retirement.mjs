...
const hardeningOffenders = [];

for (const full of files) {
  const text = fs.readFileSync(full, "utf8");
  if (text.includes("@/api/legacy/legacyApiHardening")) {
    hardeningOffenders.push(path.relative(root, full).replaceAll(path.sep, "/"));
  }
}

ok("zero legacyApiHardening imports", hardeningOffenders.length === 0, `${hardeningOffenders.length} offender(s)`);
