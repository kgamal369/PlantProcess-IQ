...
function write(relativePath, content) {
  fs.writeFileSync(p(relativePath), content.replace(/\n/g, "\r\n"), "utf8");
...
const phase1ContractFiles = [
  "Backend/PlantProcess.Api/Endpoints/Admin/Phase1WorkflowTruthEndpoints.TopLevelContracts.001.cs",
  "Backend/PlantProcess.Api/Endpoints/Admin/Phase1WorkflowTruthEndpoints.TopLevelContracts.002.cs"
];

for (const file of phase1ContractFiles) {
  if (!fs.existsSync(p(file))) {
    throw new Error("Missing expected Phase1 top-level contract file: " + file);
  }
  ...
  if (lines > 300) {
    throw new Error(file + " exceeds 300 lines: " + lines);
  }

  if (!text.includes("PPIQ_REALIZATION_T027_PHASE1_WORKFLOW_TRUTH_TOP_LEVEL_CONTRACTS")) {
    throw new Error(file + " is missing top-level contract marker.");
  }
}

const allPhase1Contracts = phase1ContractFiles.map(read).join("\n");
...
for (const signal of [
  ...
  if (!allPhase1Contracts.includes(signal)) {
    throw new Error("Phase1 top-level contracts are missing: " + signal);
  }
}
...
run("Backend build after T-027 continuation", "dotnet", ["build", "Backend"]);

console.log("");
console.log("T-027 continuation completed. If build is green, T-027 is truly done.");