...
<#
.SYNOPSIS
    PlantProcess IQ Ultimate Documentation + Deep AI Audit Generator (v2.3).
...
.DESCRIPTION
    ...
    original GeneratePlantProcessIQ_UltimateAudit.ps1 with the following goals:
      ...
      - The top-level ML\ Python runtime is exported into its own ML Runtime audit
        document, separate from Backend and Tools. Python source/tests/manifests are
        classified explicitly, while virtualenv/cache folders are hard-excluded.
      - The C# side of the ML boundary (Backend\PlantProcess.ML.Runtime plus its
        dedicated tests) is exported into a separate ML .NET Bridge audit instead of
        being misclassified as generic Tools/Misc.
      - Active deployment material under deploy\, env\profiles\ and scripts\deploy\
        is classified as Infrastructure. Historical deploy\.ppiq-backups content and
        generated pack backup/TRX folders are hard-excluded.
      - Audit-signal scanning no longer scans its own regex definitions or test/spec
        files as if they were active product defects. Real validator/tooling defects
        remain visible.
      - Python dependency manifests/locks are classified explicitly, including
        requirements.lock.
      - When secret masking is disabled, generated documents are marked INTERNAL ONLY.
...
$hardExcludedSegments = @(
    ...
    ".hypothesis", ".tox", ".nox",
    ".ppiq-backups", ".phase1_phase2_backup",
...
# Relative generated/historical paths that must never be captured, even when a
# child file would normally be force-included.
$hardExcludedRelGlobs = @(
    "deploy\.ppiq-backups\*",
    "tools\packs\backup\*",
    "tools\packs\trx\*",
    "tools\packs\_backup_*\*",
    "Frontend\PlantProcess.Web\playwright-report*\*",
    "Frontend\PlantProcess.Web\test-results*\*",
    "Website\PlantProcess.Website\playwright-report*\*",
    "Website\PlantProcess.Website\test-results*\*"
)
...
function Get-FileClassification {
    ...
    if ($RelativePath -match '^Backend[\\/]PlantProcess\.ML\.Runtime[\\/]') {
        if ($ext -eq ".csproj") { return "ML .NET Bridge Project" }
        if ($ext -eq ".cs") { return "ML .NET Bridge Source" }
    }
    if ($RelativePath -match '^Backend[\\/]tests[\\/]PlantProcess\.ML\.Runtime\.Tests[\\/]') {
        if ($ext -eq ".csproj") { return "ML .NET Bridge Test Project" }
        if ($ext -eq ".cs") { return "ML .NET Bridge Test Source" }
    }

    if ($ext -eq ".csproj") { return ".NET Project File" }
    if ($ext -eq ".cs") { return "C# Source" }
    if ($RelativePath -match '^ML[\\/]tests[\\/]') {
        if ($ext -eq ".py") { return "Python Test Source" }
        return "ML Test / Fixture"
    }
    if ($name -eq "pyproject.toml") { return "Python Project Manifest" }
    if ($name -eq "requirements.txt") { return "Python Dependency Manifest" }
    if ($name -in @("requirements.lock", "uv.lock", "poetry.lock", "Pipfile.lock")) { return "Python Dependency Lock" }
    if ($RelativePath -match '^ML[\\/](\.gitignore|\.gitattributes)$') { return "ML Project Config" }
    if ($ext -eq ".py") { return "Python Source" }
    if ($ext -eq ".pyi") { return "Python Type Stub" }
...
function Get-PrimaryCategory {
    ...
    # ML boundary gets first-class documentation on both sides.
    if ($rp -match '^Backend\\PlantProcess\.ML\.Runtime\\') { return "03B_ML_DotNet_Bridge" }
    if ($rp -match '^Backend\\tests\\PlantProcess\.ML\.Runtime\.Tests\\') { return "03B_ML_DotNet_Bridge" }
    ...
    if ($rp -match '^ML\\') { return "03A_ML_Runtime" }
    ...
    # Website owns its own build/runtime files, including its Dockerfile.
    if ($rp -match '^Website\\') { return "08_Website" }

    # Active deployment/operations material is Infrastructure. Demo SQL/CSV fixtures
    # were routed to 07A before this broad deploy rule.
    if ($rp -match '^Infrastructure\\') { return "06_Infrastructure" }
    if ($rp -match '^deploy\\') { return "06_Infrastructure" }
    if ($rp -match '^env\\profiles\\') { return "06_Infrastructure" }
    if ($rp -match '^scripts\\deploy\\') { return "06_Infrastructure" }
...
function Get-CategoryTitle {
    ...
    switch ($Category) {
        ...
        "03A_ML_Runtime" { return "ML Runtime: Python Compute, Models, Protocol, Tests" }
        "03B_ML_DotNet_Bridge" { return "ML .NET Bridge: C# Protocol, Process Runner, Cross-Language Tests" }
...
function Test-IsAuditSignalEvidenceOrTestPath {
    param([Parameter(Mandatory = $true)][string]$RelativePath)

    $rp = $RelativePath -replace '/', '\'

    # Never scan this scanner's own rule definitions as repository defects.
    if ($rp -match '^tools\\_?GeneratePlantProcessIQ_UltimateAudit(?:_v[0-9_]+)?\.ps1$') { return $true }

    # Negative tests/specs intentionally contain strings they assert against.
    if ($rp -match '^Backend\\tests\\') { return $true }
    if ($rp -match '^Frontend\\PlantProcess\.Web\\e2e\\') { return $true }
    if ($rp -match '^Website\\PlantProcess\.Website\\tests\\') { return $true }
    if ($rp -match '\.(test|spec)\.(cs|ts|tsx|js|jsx|cjs|mjs)$') { return $true }
...
function Get-AuditSignalsForContent {
    ...
    if (Test-IsAuditSignalEvidenceOrTestPath -RelativePath $RelativePath) { return $results }
...
Write-Step "============================================================"
Write-Step " PlantProcess IQ Ultimate Documentation + Deep Audit (v2.3)"
Write-Step "============================================================"
...
if (-not $MaskSecrets) {
    Write-Warn2 "SHARE SAFETY    : INTERNAL ONLY - secrets/config values are unmasked. Use -MaskSecrets `$true before external sharing."
}
Write-Step "============================================================"
...
foreach ($file in $allCandidateFiles) {
    ...
    if (Test-RelMatchesAnyGlob -RelativePath $relativePath -Globs $hardExcludedRelGlobs) {
        $skippedFiles.Add([pscustomobject]@{
            RelativePath = $relativePath; Reason = "Hard-excluded generated/historical path"; SizeBytes = $file.Length
        })
        continue
    }
...
$misclassifiedMlPythonFiles = @(
    $includedFiles | Where-Object {
        $_.RelativePath -match '^ML\\' -and $_.Category -ne "03A_ML_Runtime"
    }
)
if ($misclassifiedMlPythonFiles.Count -gt 0) {
    $badPaths = ($misclassifiedMlPythonFiles | ForEach-Object { $_.RelativePath }) -join "; "
    throw "ML Python classification integrity failure: $badPaths"
}

$misclassifiedMlBridgeFiles = @(
    $includedFiles | Where-Object {
        (
            $_.RelativePath -match '^Backend\\PlantProcess\.ML\.Runtime\\' -or
            $_.RelativePath -match '^Backend\\tests\\PlantProcess\.ML\.Runtime\.Tests\\'
        ) -and $_.Category -ne "03B_ML_DotNet_Bridge"
    }
)
if ($misclassifiedMlBridgeFiles.Count -gt 0) {
    $badPaths = ($misclassifiedMlBridgeFiles | ForEach-Object { $_.RelativePath }) -join "; "
    throw "ML .NET bridge classification integrity failure: $badPaths"
}

$demoSeedFileCount = @($includedFiles | Where-Object { $_.Category -eq "07A_DEMO_SQL_Data_Seed" }).Count
$mlPythonFileCount = @($includedFiles | Where-Object { $_.Category -eq "03A_ML_Runtime" }).Count
$mlBridgeFileCount = @($includedFiles | Where-Object { $_.Category -eq "03B_ML_DotNet_Bridge" }).Count
Write-Ok ("Collected {0} included files, {1} skipped, {2} audit signals. Demo seed: {3}; ML Python: {4}; ML .NET bridge: {5}." -f $includedFiles.Count, $skippedFiles.Count, $allSignals.Count, $demoSeedFileCount, $mlPythonFileCount, $mlBridgeFileCount)
...
function Write-Header {
    ...
    if ($MaskSecrets) {
        $Writer.WriteLine("Share Safety          : SANITIZED MODE - still review before external sharing")
    }
    else {
        $Writer.WriteLine("Share Safety          : INTERNAL ONLY - secrets/config values may be present")
    }
...
$categoryDefinitions = @(
    ...
    [pscustomobject]@{ Key = "03A_ML_Runtime"; FilePrefix = "03A_ML_Runtime"; Title = "PLANTPROCESS IQ ML RUNTIME AUDIT - PYTHON COMPUTE / MODELS / PROTOCOL / TESTS" },
    [pscustomobject]@{ Key = "03B_ML_DotNet_Bridge"; FilePrefix = "03B_ML_DotNet_Bridge"; Title = "PLANTPROCESS IQ ML .NET BRIDGE AUDIT - C# PROTOCOL / PROCESS RUNNER / CROSS-LANGUAGE TESTS" },
