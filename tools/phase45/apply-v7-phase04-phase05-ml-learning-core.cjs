...
// ============================================================================
// SQL: Phase 4/5 deterministic ML learning core
// ============================================================================

write("Backend/database/scripts/204_phase04_phase05_ml_learning_core.sql", String.raw`
-- ============================================================================
-- PlantProcess IQ
    ...
    (
        ...
        (
            SELECT
                ...
                count(*) AS n
            FROM cat_features f
            JOIN cat_outcomes o ON o.coil_id = f.coil_id
            WHERE v_family IN ('all', 'overall') OR o.family = v_family
            GROUP BY f.feature_key, f.feature_grain, f.feature_value, o.outcome_key, o.family, f.heat_id
        ) s
        JOIN cat_features f ON f.feature_key = s.feature_key AND f.feature_grain = s.feature_grain
        JOIN cat_outcomes o ON o.outcome_key = s.outcome_key AND o.family = s.family
        GROUP BY f.feature_key, f.feature_grain, o.outcome_key, o.family
    )
    INSERT INTO public.ml_learning_results_v1
    (
        run_id,
...
// ============================================================================
// API endpoint: ML learning proof surface
// ============================================================================

write("Backend/PlantProcess.Api/Endpoints/Analytics/MlLearningEndpoints.cs", String.raw`
using System.Data;
...
// ============================================================================
// Program.cs route wiring
// ============================================================================

update("Backend/PlantProcess.Api/Program.cs", (text) => {
  let next = text;

  if (!next.includes("using PlantProcess.Api.Endpoints.Analytics;")) {
    const firstUsing = next.match(/^using .+;\r?\n/m);
    if (!firstUsing) {
      throw new Error("Could not find using insertion point in Program.cs");
    }
    next = next.replace(firstUsing[0], firstUsing[0] + "using PlantProcess.Api.Endpoints.Analytics;\n");
  }

  if (!next.includes("MapMlLearningEndpoints")) {
    const markers = [
    ...
    const marker = markers.find((x) => next.includes(x));

    if (!marker) {
      throw new Error("Could not find analytics endpoint mapping marker in Program.cs");
    }
...
write("tools/validation/validate-v7-phase04-phase05-acceptance.cjs", String.raw`
const fs = require("node:fs");
...
console.log("");
console.log("PPIQ V7 Phase 4/5 ML learning core patch applied.");
console.log("");
console.log("Next commands:");
console.log("  node .\\tools\\validation\\validate-v7-phase04-phase05-acceptance.cjs");
console.log("  psql -f Backend\\database\\scripts\\204_phase04_phase05_ml_learning_core.sql");
