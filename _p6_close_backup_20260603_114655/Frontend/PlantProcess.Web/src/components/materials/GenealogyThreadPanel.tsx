    ...
    <StandardCard
      ...
      actions={
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <StandardButton variant="ghost" type="button" onClick={() => load(pending)} disabled={status === "loading"}>
            {status === "loading" ? "Resolving..." : "Investigate"}
          </StandardButton>
          <StandardButton variant="ghost" type="button" onClick={calculateRisk} disabled={!unitId || busy === "risk"}
          ...
          <StandardButton variant="primary" type="button" onClick={openPdf} disabled={!unitId || busy === "pdf"}
            ...
            <div key={i} style={{ position: "relative", marginBottom: 10 }}>
              ...
              <div style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 10, padding: "8px 12px" }}>
                <div style={{ color: C.text, fontWeight: 600, fontSize: 14 }}>
                  {label}{seq ? <span style={{ color: C.muted, fontWeight: 400 }}> · #{seq}</span> : null}
                </div>
