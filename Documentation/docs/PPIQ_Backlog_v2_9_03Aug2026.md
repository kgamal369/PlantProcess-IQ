...
**Version 2.9 - 3 August 2026**  
**Supersedes:** v2.8  
**Trigger:** The agreed M1 presentation fast path is now codified in Chapter 3 section 4.5.2a, and five M1-P1b contracts still instructed the full live pipeline inside M1. They are rewritten, the M1 to M2 convergence gets its own emitter task, and two phases are rebalanced so no estimate is shaved to fit.

---

## 0. WHAT v2.9 CHANGES, AND WHY

Five contracts rewritten, one added, two phases rebalanced. No estimate shaved.

**The finding.** Chapter 3 section 4.5.2a now fixes the M1 presentation fast path: the presentation database is materialised from one frozen Fleet v2 generator, Docker is connection-test only during M1, and the full external path returns as the sole authority at M2a. Five M1-P1b contracts still instructed a developer to regenerate the fixtures, reload the source engines, import to staging, publish the mappings and project to canonical inside M1. A backlog that instructs a different path from the design is how the last three weeks of confusion happened.

**The correction.** Those five, 34 hours, are replaced by six contracts totalling 44 hours: merge the best existing material into one Fleet v2 truth, scale it, materialise the presentation canonical operational entities, compute the analysis entities with the real engines, materialise the source-shaped staging representation, and certify cross-layer consistency. The last two open M1-P2 because they are the direct prerequisites of the authoring shell.

**The arithmetic, stated rather than fitted.** 34 out and 44 in would have put M1-P1b at 128, eight hours above the ceiling. Rather than trim estimates or invent a short phase, the staging and certification contracts moved to the front of M1-P2. M1-P1b lands at 114 and M1-P2 at 103, both inside the band. M1 scope rises 10 hours to 570.

**At the convergence point.** A dedicated emitter task now precedes the clean-room rebuild, because emitting native fixtures and proving the full path from them prove different things. To make room without shaving, the two API namespace migration contracts move from M2a-P1 to M2a-P3, where platform and runtime hardening belongs: M2a-P1 lands at 114 and M2a-P3 at 106.

**One inference withdrawn.** The clean-room contract cited the T-010 reading that 106,272 canonical rows against 16,640 staged proved direct canonical seeding. Measurement retracted it. The M1 database held three generations of one fleet at once, and the clean-room task exists to eliminate that generation drift.
...
## 4. M1 IS 570 HOURS OF BASELINE SCOPE, 478 OF THEM REMAINING
...
**Programme:** M1 570 + M2a 432 + M2b 233 = **1,235 hours before M3**, plus 204 reserved, for **1,439 total**. 167 tasks, no task over 12 hours, all fourteen phases inside 80 to 120.
...
| **M1-P1b** | M1 | Presentation Fleet v2 - reconcile, extend, regenerate, prove | 17 | 114 |
| **M1-P2** | M1 | No-Code Authoring Shell - wiring, SQL and widget authoring | 11 | 103 |
...
# M1 - CUSTOMER PRESENTATION (570 h baseline scope, 478 h remaining)
...
**17 tasks / 114 hours.** Reconcile what the emulation actually contains against what the 36 charts require, extend the existing generator rather than replacing it, regenerate through the full product path, and only then re-measure and close the reopened matrix.
...
### T-031 - Certify cross-layer consistency across staging, canonical and analysis
...
### T-146 - Retire the superseded correlation engine
...
|---|---:|---|
| M1 baseline scope | 570 | every task in M1, complete or not |
| Completed | 92 | T-001 to T-013 - all of M1-P1, plus the reconciliation that opens M1-P1b |
| Reopened | 0 | T-007 and T-008 closed as pre-generation specifications |
| Remaining estimate | 478 | scope minus completed |

Reporting one number invites the reading that M1 grew on top of finished work.
