...
**Supersedes:** v2.3  
**Trigger:** the dataset re-assessment went deeper, and it exposed an ordering defect of mine that my own guard could not see.
...
> Current emulation inventory, then the chart blueprint, then map each chart to existing fields and phenomena, then classify, then change the generator **only for true gaps**, then regenerate, then statistical QA, then import through the product, then certify.
...
**Programme:** M1 455 + M2a 404 + M2b 233 = **1,092 hours before M3**, plus 204 reserved. 149 tasks, no task over 12 hours, all thirteen phases inside 80 to 120.
...
|---|---|---|---:|---:|
| **M1-P1** | M1 | Presentation Truth and Dataset Foundation | 19 | 118 |
...
Priority mix: Critical 89, Very Important 54, Important 8, Optional 1

---


# M1 - CUSTOMER PRESENTATION (455 h)


## PHASE M1-P1 - Presentation Truth and Dataset Foundation

**19 tasks / 118 hours.** Establish what will be shown, prove the environment rebuilds from source control, and plant the analytical phenomena the whole demonstration feeds on. Blocks M1-P3 and M1-P5.
...
### T-010 - Close the named field gaps: shift attribute, grade specification, downtime quantities
...
**Description.** The three field-level gaps the matrix names. A SHIFT attribute if the sources do not already carry one derivable from the production timestamp. A GRADE SPECIFICATION source table carrying per-grade chemistry minima and maxima, so the conditional-formatting demonstration draws its limits from the customer's own data and never from a literal in the product. And the two downtime quantities populated independently once the schema slice lands. NOTE ON BUFFER ABSORPTION: the visible story is equipment stopped 145 minutes, production impact 92, buffer absorbed 53 - but buffer absorbed is stopped minus impact and is DERIVED, not stored. Do not add a third column that can contradict the other two.

**Validation.** A SQL script per gap printing the field and its distribution. For the grade specification, at least one heat outside its band and one inside, so the conditional format has something to show. For downtime, equipment stoppages outnumber production stoppages and at least one row has stopped minutes above zero with production impact zero.

### T-011 - Shift and crew operating-practice regimes
...
### T-012 - Post-maintenance recovery and campaign-ageing regimes
...
### T-013 - Equipment personality and temporal regime changes
...
### T-014 - Regenerate and prove every phenomenon in the matrix
...
**Description.** Regenerate the emulated sources with the enrichments, then run the assertion every phenomenon carries in T-008. This is the task that stops the enrichment quietly breaking what already worked. The seventeen existing relations and their negative controls must still verify at their previous strength, and the new regimes must verify at theirs. THE ORDERING RULE THIS TASK ENFORCES, and it is the reason the matrix is built before the generator is touched: adding data without an inventory first can easily damage relations that were already good. Never modify the generator ahead of the coverage audit.

**Validation.** Every row of the matrix passes its own assertion. The negative controls still show no association - a control that starts correlating is a generator defect and blocks the regeneration. Produce a before-and-after table of every planted relation's measured effect so a weakened relation is visible rather than discovered on a dashboard later.

### T-015 - Verify the confounded correlation and the insufficient-support refusal
...
### T-016 - Run the canonical semantic path end to end through the M1 compatibility boundaries
...
### T-017 - Five-layer realism audit of the emulated plant
...
### T-018 - Establish and fix the architecture test pool reliability
...
### T-019 - Canonicalise the JourneyRail to J1 to J15
...
### T-020 - Shared Authoring Shell, part 1: the shell contract and the four regions
...
### T-021 - Shared Authoring Shell, part 2: relational block grammar on the board
...
### T-022 - Registry-driven schema, table and attribute tree
...
### T-023 - Compiled-SQL pane and debug log with rows and cost
...
### T-024 - SQL mode: safe editor, run test, returned columns and the reconstructability rule
...
### T-025 - Certify returned-column role mapping inside the S2 shell
...
### T-026 - Add Widget and Edit Widget open the shared shell in S2 mode
...
### T-027 - Final definition service interface with a compatibility adapter
...
### T-028 - Authoring states, keyboard path, RTL and error wording
...
### T-029 - D2 Page Builder, part 1: create a page and reach the shared shell
...
### T-030 - D2 Page Builder, part 2: arrange, save layout and publish
...
### T-031 - Bring the workspace to the final D1 anatomy
...
### T-032 - Certify the three operational dashboards and fix their bindings
...
### T-033 - Certify the analysis and model dashboards and choose the six shown
...
### T-034 - Register the final chart grammar and implement the presentation subset
...
### T-035 - Give the seven pages distinct visual grammars from the registered grammar
...
### T-036 - Associative model, part 1: the alternative state and registry-driven fields
...
### T-037 - Certify layout drag, resize, save, reload and responsive behaviour
...
### T-038 - Drill to population, provenance and evidence
...
### T-039 - Widget failure isolation and the seven states
...
### T-040 - Remove the hardcoded parameter default from the API client
...
### T-041 - Reduce the demonstration navigation and add the inventory ratchet
...
### T-042 - J4 Connections: read-only proof and load budget made visible
...
### T-043 - J5 and J6 Dataset registry browse and watermark suggestion
...
### T-044 - J6 Import progress visibility
...
### T-045 - J7 Relationship model vertical slice, part 1: publish one relationship
...
### T-046 - J7 Relationship model vertical slice, part 2: one resolver consumer
...
### T-047 - Associative model, part 2: cross-source state through the published relationship
...
### T-048 - C6 Relationship Browser, minimal read-only slice
...
### T-049 - C2 Mapping Health, part 1: the typed issue contract and the reprocess API
...
### T-050 - C2 Mapping Health, part 2: the final visible page
...
### T-051 - C5 Genealogy: converge the legacy workbench onto the final two-state surface
...
### T-052 - Add job_definitions.target_definition_id and the JB error codes
...
### T-053 - J12 Analysis authoring: converge onto D3 Analysis Toolbox in S3 mode
...
### T-054 - One visible readiness authority on Home and Analysis
...
### T-055 - Findings evidence panel, registry-driven throughout
...
### T-056 - Retire the hardcoded outcome and grain arrays
...
### T-057 - Website, part 1: the five-product information architecture
...
### T-058 - Website, part 2: polish the presentation routes
...
### T-059 - Build the G1 persistent assistant dock
...
### T-060 - Page and widget context envelope
...
### T-061 - Add the page and widget chunk family to the retrieval corpus
...
### T-062 - Registry-typed quantity guard on assistant answers
...
### T-063 - Citation chips, evidence strip and suggested questions
...
### T-064 - Certified question pack and offline fallback
...
### T-065 - One Playwright journey covering all six beats
...
### T-066 - Execute visual regression and accessibility on the presented routes
...
### T-068 - Capture the Customer Contract Continuity snapshots
...
### T-069 - Write the screen-by-screen demonstration script
...
### T-070 - Presentation environment preparation and clean-start verification
...
### T-071 - Three rehearsals, hostile hands and the fallback package
...
### T-072 - Physical three-schema migration
...
### T-073 - Canonical migration order and legacy script archival
...
### T-074 - definition_store, definition_versions and definition_dependencies
...
### T-075 - Move all five definition kinds onto the store
...
### T-076 - Impact preview, export and import
...
### T-077 - Registry authority: dimensions and measures as rows
...
### T-078 - Plant-vocabulary sweep, part 1: build the term list and the architecture test
...
### T-079 - Plant-vocabulary sweep, part 2: clear the violations and rename the canonical grain
...
### T-080 - API namespace migration, part 1: map the 92 prefixes onto the 27 domains and stand up dual-serve
...
### T-081 - API namespace migration, part 2: migrate the clients and add the token gate
...
### T-082 - Relationship members, cardinality, grain conversion and preferred paths
...
### T-083 - Path resolver, part 1: resolver core and the first eight consumers
...
### T-084 - Path resolver, part 2: the remaining eight consumers and the regression suite
...
### T-085 - Relationship Browser page and path evidence
...
### T-086 - Quarantine, part 1: the table, the reprocess API and the first eight PV classes
...
### T-087 - Quarantine, part 2: the remaining seven PV classes and per-class tests
...
### T-088 - Quarantine retry, reprocess and Mapping Health completion
...
### T-089 - Identity resolution across sources
...
### T-090 - Genealogy bidirectional walk hardening and weight proof
...
### T-091 - Projection through the versioned mapping, with version stamping
...
### T-092 - Idempotent reprojection and mapping-version regression
...
### T-093 - Job target version policy and dependency DAG
...
### T-094 - Weighted pools, compute weights and admission control
...
### T-095 - stage_watermarks and delta-scoped projection
...
### T-096 - Delta-scoped feature refresh and analysis, with telemetry
...
### T-097 - Chunk manifests, checkpoint, resume and deterministic merge
...
### T-098 - Scan budget and the Scan Amplification metric
...
### T-099 - Force RLS on every tenant-owned table with an architecture test
...
### T-100 - Secret and configuration hygiene
...
### T-101 - Tenant keys, tenant-aware uniqueness and canonical namespace on new APIs
...
### T-102 - Fresh-install Rule 2 acceptance test, ephemeral
...
### T-103 - J1 to J3 commissioning built for real
...
### T-104 - Eight-role catalogue with three enforcement layers
...
### T-105 - Users and Roles administration surface
...
### T-106 - Licence and entitlement enforcement
...
### T-107 - Container architecture and configuration profiles
...
### T-108 - Install package, migration runner, upgrade and rollback
...
### T-109 - Backup with a tested restore acceptance procedure
...
### T-110 - Minimum monitoring, health and alerting
...
### T-111 - Support runbook and UAT dataset and configuration import
...
### T-112 - Canonical journey regression and the Continuity comparison
...
### T-113 - Feature store, outcome store and snapshots
...
### T-114 - Compute runs and correlation result persistence
...
### T-115 - Model registry, serving identity and fallback
...
### T-116 - Practice signature, windowing, context and cohorts
...
### T-117 - Support, confidence, back-off ladder and tolerance sensitivity
...
### T-118 - practice_statistics persistence, drift and D10 Practice Insights
...
### T-119 - Bindable intelligence registry and evidence handles
...
### T-120 - Tenant-aware uniqueness across the intelligence tables
...
### T-121 - Incremental practice recomputation
...
### T-122 - Wire a real model behind the assistant provider seam
...
### T-123 - Assistant finalisation on canonical sources
...
### T-124 - prediction_runs, predictions and prediction_current
...
### T-125 - Prediction drivers and comparables, persisted
...
### T-126 - Actionable deadline and latency health
...
### T-127 - Remediation candidate generation from the customer's own history
...
### T-128 - The per-prediction nine-check eligibility gate, can_accept and suppression
...
### T-129 - Accept, Reject and Defer with action recording
...
### T-130 - Outcome capture, evaluation and escalation
...
### T-131 - Retire the superseded correlation engine
...
### T-132 - Fix the outcome namespace, grain assignment and ordinal loader
...
### T-133 - Map the 108 page files onto the 40 target pages
...
### T-134 - Delete the legacy redirects and re-verify continuity
...
### T-137 - Customer data edge cases
...
### T-138 - Connector certification against real sources
...
### T-139 - Query plans, indexes and partition boundaries
...
### T-140 - Pool weights, scan amplification and model-serving memory
...
### T-141 - Customer definitions built through the product
...
### T-142 - Practice and prediction calibration
...
### T-143 - Remediation validation against real process constraints
...
### T-144 - C1 to C4 capacity certification
...
### T-145 - HA, DR and restore rehearsal
...
### T-146 - SSO and identity integration
...
### T-147 - Site security hardening and sign-off
...
### T-148 - Monitoring, SLOs and support escalation
...
### T-150 - Commercial capacity finalisation and the sales calculator
...
### T-151 - Five-product website production completion
...
### T-152 - Documentation, training and production acceptance
