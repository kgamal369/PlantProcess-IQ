...
**Version 2.7 - 3 August 2026**  
**Supersedes:** v2.6.1  
**Trigger:** Presentation Fleet v2 changes job at the M1 boundary. It stops being disposable presentation data and becomes the controlled reference plant that every M2 intelligence feature is developed and benchmarked against - so it needs a real entry task in M2a, not a prose note.
...
**Programme:** M1 552 + M2a 422 + M2b 233 = **1,207 hours before M3**, plus 204 reserved, for **1,411 total**. 164 tasks, no task over 12 hours, all fourteen phases inside 80 to 120.
...
| **M2a-P1** | M2a | Canonical Schema Authority and the Unified Definition Store | 12 | 118 |
...
Priority mix: Critical 101, Very Important 54, Important 8, Optional 1
...
# M2a - DEPLOYABLE CORE, ENDS WITH THE ON-SITE INSTALLATION (422 h)


## PHASE M2a-P1 - Canonical Schema Authority and the Unified Definition Store

**12 tasks / 118 hours.** Replace the M1 compatibility adapter with the real definition store and move the schema to its final three-schema topology, without the customer-visible contract moving.

### T-082 - Clean-room rebuild of the Fleet v2 emulator sources from source control
...
### T-083 - Freeze and certify Fleet v2 as the M2 reference validation dataset
...
### T-085 - Canonical migration order and legacy script archival
...
### T-086 - definition_store, definition_versions and definition_dependencies
...
### T-087 - Move all five definition kinds onto the store
...
### T-088 - Impact preview, export and import
...
### T-089 - Registry authority: dimensions and measures as rows
...
### T-090 - Plant-vocabulary sweep, part 1: build the term list and the architecture test
...
### T-091 - Plant-vocabulary sweep, part 2: clear the violations and rename the canonical grain
...
### T-092 - API namespace migration, part 1: map the 92 prefixes onto the 27 domains and stand up dual-serve
...
### T-093 - API namespace migration, part 2: migrate the clients and add the token gate
...
### T-094 - Relationship members, cardinality, grain conversion and preferred paths
...
### T-095 - Path resolver, part 1: resolver core and the first eight consumers
...
### T-096 - Path resolver, part 2: the remaining eight consumers and the regression suite
...
### T-097 - Relationship Browser page and path evidence
...
### T-098 - Quarantine, part 1: the table, the reprocess API and the first eight PV classes
...
### T-099 - Quarantine, part 2: the remaining seven PV classes and per-class tests
...
### T-100 - Quarantine retry, reprocess and Mapping Health completion
...
### T-101 - Identity resolution across sources
...
### T-102 - Genealogy bidirectional walk hardening and weight proof
...
### T-103 - Projection through the versioned mapping, with version stamping
...
### T-104 - Idempotent reprojection and mapping-version regression
...
### T-105 - Job target version policy and dependency DAG
...
### T-106 - Weighted pools, compute weights and admission control
...
### T-107 - stage_watermarks and delta-scoped projection
...
### T-108 - Delta-scoped feature refresh and analysis, with telemetry
...
### T-109 - Chunk manifests, checkpoint, resume and deterministic merge
...
### T-110 - Scan budget and the Scan Amplification metric
...
### T-111 - Force RLS on every tenant-owned table with an architecture test
...
### T-112 - Secret and configuration hygiene
...
### T-113 - Tenant keys, tenant-aware uniqueness and canonical namespace on new APIs
...
### T-114 - Fresh-install Rule 2 acceptance test, ephemeral
...
### T-115 - J1 to J3 commissioning built for real
...
### T-116 - Eight-role catalogue with three enforcement layers
...
### T-117 - Users and Roles administration surface
...
### T-118 - Licence and entitlement enforcement
...
### T-119 - Container architecture and configuration profiles
...
### T-120 - Install package, migration runner, upgrade and rollback
...
### T-121 - Backup with a tested restore acceptance procedure
...
### T-122 - Minimum monitoring, health and alerting
...
### T-123 - Support runbook and UAT dataset and configuration import
...
### T-124 - Canonical journey regression and the Continuity comparison
...
### T-125 - Feature store, outcome store and snapshots
...
### T-126 - Compute runs and correlation result persistence
...
### T-127 - Model registry, serving identity and fallback
...
### T-128 - Practice signature, windowing, context and cohorts
...
### T-129 - Support, confidence, back-off ladder and tolerance sensitivity
...
### T-130 - practice_statistics persistence, drift and D10 Practice Insights
...
### T-131 - Bindable intelligence registry and evidence handles
...
### T-132 - Tenant-aware uniqueness across the intelligence tables
...
### T-133 - Incremental practice recomputation
...
### T-134 - Wire a real model behind the assistant provider seam
...
### T-135 - Assistant finalisation on canonical sources
...
### T-136 - prediction_runs, predictions and prediction_current
...
### T-137 - Prediction drivers and comparables, persisted
...
### T-138 - Actionable deadline and latency health
...
### T-139 - Remediation candidate generation from the customer's own history
...
### T-140 - The per-prediction nine-check eligibility gate, can_accept and suppression
...
### T-141 - Accept, Reject and Defer with action recording
...
### T-142 - Outcome capture, evaluation and escalation
...
### T-143 - Retire the superseded correlation engine
...
### T-144 - Fix the outcome namespace, grain assignment and ordinal loader
...
### T-145 - Map the 108 page files onto the 40 target pages
...
### T-146 - Delete the legacy redirects and re-verify continuity
...
### T-149 - Customer data edge cases
...
### T-150 - Connector certification against real sources
...
### T-151 - Query plans, indexes and partition boundaries
...
### T-152 - Pool weights, scan amplification and model-serving memory
...
### T-153 - Customer definitions built through the product
...
### T-154 - Practice and prediction calibration
...
### T-155 - Remediation validation against real process constraints
...
### T-156 - C1 to C4 capacity certification
...
### T-157 - HA, DR and restore rehearsal
...
### T-158 - SSO and identity integration
...
### T-159 - Site security hardening and sign-off
...
### T-160 - Monitoring, SLOs and support escalation
...
### T-162 - Commercial capacity finalisation and the sales calculator
...
### T-163 - Five-product website production completion
...
### T-164 - Documentation, training and production acceptance
