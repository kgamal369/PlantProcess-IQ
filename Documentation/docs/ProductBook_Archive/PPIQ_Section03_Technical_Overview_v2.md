...
*File maps to PPIQ.txt section 3, labelled "Chapter 2: Technical overview".*
...
| 5 | **Recommendation** | Suggested actions at later stages to avoid the predicted outcome | Pro Plus |
...
**Thirty-four pages in six groups.** Every page publishes the page contract (Section 4.7) and is specified control by control in Section 4.4. The overview below is what a manager or engineer needs in order to know why the page exists and when they will open it.
...
**E1 Assistant** - `/assistant`. Plain-language questions answered from tool results and permission-scoped retrieval, every number carrying a resolvable citation, and a refusal where evidence is absent. It exists so that a user who cannot build a dashboard or read a statistical chart can still reach an answer.
