...
**Version 2.2 - FREEZE CANDIDATE - 2 August 2026**  
**Supersedes:** v2.1, which superseded the v2.0 freeze candidate  
**Status:** the final consistency pass is applied. **Every task reference now resolves mechanically and every dependency provably runs earlier** - see section 3.

---

## 1. THE SEVEN CONSISTENCY FIXES, ALL ACCEPTED

| # | Finding | Fix |
|---|---|---|
| **1** | **Stale task references after renumbering.** Two tasks named the compatibility adapter by an id that now belongs to role-mapping certification, and one named the relationship slice by ids that had moved | **The class is closed by construction, not by correction.** Descriptions no longer contain task ids at all. They carry `{{ref:Task Name}}`, resolved after ordering. A generator guard refuses to emit if any reference fails to resolve **or if any hardcoded id survives anywhere in the corpus** |
| **2** | **Two execution-order violations.** The six-grammar task ran before the chart registry it selects from, and the associative task depended on a resolver built two phases later | The chart registry now runs first. The associative task is **split by dependency**: alternative state and registry-driven fields in M1-P3, cross-source relationship-backed state in M1-P4 immediately after the resolver. **The guard now asserts every referenced task has a lower id**, and it caught a third violation I had introduced while writing this pass |
| **3** | **Widget kinds treated as customer-extensible.** The validation said add a kind through the registry and see it appear | **Corrected.** Widget kinds are structural product grammar and are fixed, like chart types and the numeric safety limits. What is customer-driven is dimensions, measures, filters and data. Validation is now: the endpoint returns the final allowed structural kinds, and the reducer holds no compiled widget library |
| **4** | **`IDefinitionService` was missing S4.** The kind enum listed Transformation, Page, Widget, Analysis and LogRule | **Corrected and widened.** The enum now carries Model (S4) plus the S2 sub-kinds the design also versions - MasterDimension, MasterMeasure, Filter, Hierarchy, Bookmark. Eleven members, asserted by a test. An external contract that has to gain a member in M2a was never final |
| **5** | **The traceability matrix still listed `AnalysisJobConfigPage` as a presentation screen** | Row removed. The D3 and S3 convergence task already ruled it out of M1 navigation, so there was no open question left to record. Matrix reissued as v1.1 |
| **6** | **A2 Home and C6 Relationship Browser left as unresolved scope** | **Home is removed from the M1 screen list.** J1 to J3 are narrated as commissioning, so the screen J1 lands on is never opened - this closes the question without adding six hours. C6 needs a decision at sign-off: a minimal read-only page, or not shown |
| **7** | **Website wording residue.** M1-P4 was still named for a website beat it no longer carries, and the M3 website task still described work M1 now does | M1-P4 renamed. The M3 task becomes **production completion of the five product pages** - full content, proof, SEO, analytics, lead flows, localization and final honesty certification |

---

## 2. M1 IS 452 HOURS

v2.1 stated 450. The associative split in fix 2 costs two hours, because the same work in two places carries a small seam. **The three options from v2.1 stand unchanged** and are restated here so the decision is not lost:

| Option | Shape | What it costs |
|---|---|---|
| **A** | **Accept 452 h** | One more day of team capacity, or one more pair of hands. Recommended if the date is fixed and the team is not |
| **B** | **Cut ~42 h to land at 410** | Real scope leaves. The ranked menu below names what the customer loses on each line |
| **C** | **Accept 446 h** | Move only `ppiq_acceptance_empty` and the findings registry default to M2a. Nothing the customer sees changes |

**Never cut, in any option:** the rehearsals, the failure injection, the continuity snapshots, the demonstration script, the phenomena manifest, the shared shell convergence, D2 Page Builder, and the website architecture correction. Each of those trades either the presentation itself or the contract the customer is shown.

### The cut menu, ranked by what the customer notices least

| # | Cut | Hours | What is lost |
|---|---|---:|---|
| 1 | Move `ppiq_acceptance_empty` to M2a | 4 | The M1 release gate. Nothing visible |
| 2 | Defer the findings registry-driven default to M2a | 2 | A hardcoded initial outcome survives one milestone |
...
| 6 | Defer the associative Alternative state to M2a | 4 | Three states instead of four in the strip |
| 7 | Genealogy: overlay the two-state surface, defer full convergence | 4 | Legacy panels remain reachable behind the demo path |
...
## 3. THE GUARD THAT MAKES THIS FREEZABLE

The stale-reference defect was mechanical, and a mechanical defect deserves a mechanical fix rather than a careful proofread. The generator now refuses to emit this document unless all three hold:

1. **No task description or validation contains a hardcoded task id.** References are written as `{{ref:Task Name}}` and resolved after ordering.
2. **Every reference resolves** to a task that exists.
3. **Every referenced task has a lower id than the task referring to it** - because Task Id is planned execution order, and a task may not depend on one that runs after it.

Rule 3 earned its place immediately: it caught a violation I had introduced *while writing this very pass*, where a phase-one task referenced an adapter built in phase two. That task is now reworded to state the sequencing openly rather than to assume it.
...
| **M1-P3** | M1 | BI Workspace and Six Showcase Pages | 12 | 84 |
| **M1-P4** | M1 | Journey J4 to J15 and the Engine Slice | 14 | 98 |
...
Priority mix: Critical 86, Very Important 51, Important 7, Optional 1

---


# M1 - CUSTOMER PRESENTATION (452 h - see section 2)
...
### T-024 - Authoring states, keyboard path, RTL and error wording
...
**12 tasks / 84 hours.** Presentation beat 2. Seven dashboards are already seeded; this phase is certification, differentiation and interaction quality, not construction.

### T-025 - D2 Page Builder, part 1: create a page and reach the shared shell
...
### T-030 - Register the final chart grammar and implement the presentation subset
...
### T-031 - Give the six pages six distinct visual grammars from the registered grammar
...
**Description.** ORDERED IN v2.2 to run AFTER T-030, because the six grammars are chosen from the FINAL registered grammar, not from whichever types happen to be implemented today. Six pages that are all bar charts read as one page shown six times. Assign each of the six a dominant visual language and adjust widget definitions to match: KPI tiles with trend; Pareto; stacked or combination; scatter; heatmap; distribution or box plot; table with conditional formatting; timeline or genealogy where appropriate.

**Validation.** Place the six pages side by side as screenshots. A reviewer who has not seen the product must be able to say what each page is for from the shapes alone, without reading the titles. Every chart type used appears in the registry as implemented, never as unavailable.

### T-032 - Associative model, part 1: the alternative state and registry-driven fields
...
**14 tasks / 98 hours.** Presentation beats 3 and 4. RENAMED IN v2.2: the website tasks live in M1-P5, so this phase no longer carries beat 6. Every visible transition credible with no dead end. J1 to J3 are commissioning per Chapter 5 and are narrated, not demonstrated.
...
### T-042 - Associative model, part 2: cross-source state through the published relationship
...
### T-043 - C2 Mapping Health, part 1: the typed issue contract and the reprocess API
...
### T-044 - C2 Mapping Health, part 2: the final visible page
...
### T-045 - C5 Genealogy: converge the legacy workbench onto the final two-state surface
...
### T-046 - Add job_definitions.target_definition_id and the JB error codes
...
### T-047 - J12 Analysis authoring: converge onto D3 Analysis Toolbox in S3 mode
...
### T-048 - One visible readiness authority on Home and Analysis
...
### T-049 - Findings evidence panel, registry-driven throughout
...
### T-050 - Retire the hardcoded outcome and grain arrays
...
### T-051 - Website, part 1: the five-product information architecture
...
### T-052 - Website, part 2: polish the presentation routes
...
### T-053 - Build the G1 persistent assistant dock
...
### T-054 - Page and widget context envelope
...
### T-055 - Add the page and widget chunk family to the retrieval corpus
...
### T-056 - Registry-typed quantity guard on assistant answers
...
### T-057 - Citation chips, evidence strip and suggested questions
...
### T-058 - Certified question pack and offline fallback
...
### T-059 - One Playwright journey covering all six beats
...
### T-060 - Execute visual regression and accessibility on the presented routes
...
### T-062 - Capture the Customer Contract Continuity snapshots
...
### T-063 - Write the screen-by-screen demonstration script
...
### T-064 - Presentation environment preparation and clean-start verification
...
### T-065 - Three rehearsals, hostile hands and the fallback package
...
### T-066 - Physical three-schema migration
...
### T-067 - Canonical migration order and legacy script archival
...
### T-068 - definition_store, definition_versions and definition_dependencies
...
### T-069 - Move all five definition kinds onto the store
...
### T-070 - Impact preview, export and import
...
### T-071 - Registry authority: dimensions and measures as rows
...
### T-072 - Plant-vocabulary sweep, part 1: build the term list and the architecture test
...
### T-073 - Plant-vocabulary sweep, part 2: clear the violations and rename the canonical grain
...
### T-074 - API namespace migration, part 1: map the 92 prefixes onto the 27 domains and stand up dual-serve
...
### T-075 - API namespace migration, part 2: migrate the clients and add the token gate
...
### T-076 - Relationship members, cardinality, grain conversion and preferred paths
...
### T-077 - Path resolver, part 1: resolver core and the first eight consumers
...
### T-078 - Path resolver, part 2: the remaining eight consumers and the regression suite
...
### T-079 - Relationship Browser page and path evidence
...
### T-080 - Quarantine, part 1: the table, the reprocess API and the first eight PV classes
...
### T-081 - Quarantine, part 2: the remaining seven PV classes and per-class tests
...
### T-082 - Quarantine retry, reprocess and Mapping Health completion
...
### T-083 - Identity resolution across sources
...
### T-084 - Genealogy bidirectional walk hardening and weight proof
...
### T-085 - Projection through the versioned mapping, with version stamping
...
### T-086 - Idempotent reprojection and mapping-version regression
...
### T-087 - Job target version policy and dependency DAG
...
### T-088 - Weighted pools, compute weights and admission control
...
### T-089 - stage_watermarks and delta-scoped projection
...
### T-090 - Delta-scoped feature refresh and analysis, with telemetry
...
### T-091 - Chunk manifests, checkpoint, resume and deterministic merge
...
### T-092 - Scan budget and the Scan Amplification metric
...
### T-093 - Force RLS on every tenant-owned table with an architecture test
...
### T-094 - Secret and configuration hygiene
...
### T-095 - Tenant keys, tenant-aware uniqueness and canonical namespace on new APIs
...
### T-096 - J1 to J3 commissioning built for real
...
### T-097 - Eight-role catalogue with three enforcement layers
...
### T-098 - Users and Roles administration surface
...
### T-099 - Licence and entitlement enforcement
...
### T-100 - Container architecture and configuration profiles
...
### T-101 - Install package, migration runner, upgrade and rollback
...
### T-102 - Backup with a tested restore acceptance procedure
...
### T-103 - Minimum monitoring, health and alerting
...
### T-104 - Support runbook and UAT dataset and configuration import
...
### T-105 - Canonical journey regression and the Continuity comparison
...
### T-106 - Feature store, outcome store and snapshots
...
### T-107 - Compute runs and correlation result persistence
...
### T-108 - Model registry, serving identity and fallback
...
### T-109 - Practice signature, windowing, context and cohorts
...
### T-110 - Support, confidence, back-off ladder and tolerance sensitivity
...
### T-111 - practice_statistics persistence, drift and D10 Practice Insights
...
### T-112 - Bindable intelligence registry and evidence handles
...
### T-113 - Tenant-aware uniqueness across the intelligence tables
...
### T-114 - Incremental practice recomputation
...
### T-115 - Wire a real model behind the assistant provider seam
...
### T-116 - Assistant finalisation on canonical sources
...
### T-117 - prediction_runs, predictions and prediction_current
...
### T-118 - Prediction drivers and comparables, persisted
...
### T-119 - Actionable deadline and latency health
...
### T-120 - Remediation candidate generation from the customer's own history
...
### T-121 - The per-prediction nine-check eligibility gate, can_accept and suppression
...
### T-122 - Accept, Reject and Defer with action recording
...
### T-123 - Outcome capture, evaluation and escalation
...
### T-124 - Retire the superseded correlation engine
...
### T-125 - Fix the outcome namespace, grain assignment and ordinal loader
...
### T-126 - Map the 108 page files onto the 40 target pages
...
### T-127 - Delete the legacy redirects and re-verify continuity
...
### T-130 - Customer data edge cases
...
### T-131 - Connector certification against real sources
...
### T-132 - Query plans, indexes and partition boundaries
...
### T-133 - Pool weights, scan amplification and model-serving memory
...
### T-134 - Customer definitions built through the product
...
### T-135 - Practice and prediction calibration
...
### T-136 - Remediation validation against real process constraints
...
### T-137 - C1 to C4 capacity certification
...
### T-138 - HA, DR and restore rehearsal
...
### T-139 - SSO and identity integration
...
### T-140 - Site security hardening and sign-off
...
### T-141 - Monitoring, SLOs and support escalation
...
### T-143 - Commercial capacity finalisation and the sales calculator
...
### T-144 - Five-product website production completion
...
### T-145 - Documentation, training and production acceptance
