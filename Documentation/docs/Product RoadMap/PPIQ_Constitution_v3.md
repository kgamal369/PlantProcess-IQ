...
**Version 3.0 | 25 July 2026 | Author: Karim, SOU Industrial Software, Dusseldorf**
...
| `PPIQ_Constitution_Amendment_A1_LowCode_Shell.md` | **Absorbed in v3.0.** Its ruling replaces Part II.6 in full and reconciles Part III.14.4. The amendment file is retired. |
...
v3.0 exists for one reason: the authoring layer was the most compressed chapter in v2.0 and the most consequential one in the product. It has been rewritten in full from the author's own clarification of the M1 rule.

**Part II.6 is replaced entirely.** It now states that there is **one shell** serving five purposes rather than five surfaces that resemble each other; specifies the **two modes** and exactly what each presents; specifies the **three-level schema tree**, the **block palette groups**, and the **debug log with three described severities**; and rules that **illegal wiring is refused at drag time** with the illegal set enumerated per surface.

**Two clauses are new and neither existed in any prior document.**

The first, 6.5, resolves what looked like a contradiction between the palette specification and the layer-separation principle. It is resolved not by compromise but by a property of the journey: **the join is declared once, in S1, and never again**, because each customer knows his own plant and the vendor cannot know every customer's schema architecture. Relations are created on S1, so S1's board carries datasets; S2 to S5 operate on one prepared dataset with a single row context, so their boards may carry values and the arithmetic blocks sit on them directly. One shell, board semantics following purpose, exactly as the palette and left panel already do.

The second, 6.7, closes a question the product had never answered: **how a user selects the dataset a widget displays.** The answer is that the same shell opens in S2 mode, carrying the widget's current definition already loaded, whether the user is editing an existing widget, dropping a new one, or filling a new page. There is no second door.

**Part III.14.4 is rewritten** to align the dual-mode contract with the mode toggle of II.6.2, and to state the fork and reconstruction limits at the point of the switch rather than in a specification the user never reads.

**Each new clause carries its measured status.** Where a clause is written and not implemented, the chapter says so, names what is actually in the code today, and places the work in a milestone. Section 6.8 is that table.
...
## 6. The Five Low-Code Authoring Surfaces and the Shared Shell

*Rewritten in v3.0 from Amendment A1. The v2.0 statement was compressed and lost the two properties that matter most: that one shell serves all five purposes, and that this same shell is what opens when a user binds data to a widget.*

### 6.1 The ruling

There is **one authoring shell**. It is not five surfaces that resemble each other. It is one surface whose left panel, palette, board semantics and validator are parameterised by which of the five authoring purposes it was opened for.

A user who learns the shell once has learned every authoring act in the product. This is the single largest usability decision in the platform and it is constitutional.

| Purpose | What is authored | Palette presented |
|---|---|---|
| **S1 Data preparation** | Staged data prepared, filtered, linked, aliased and mapped into the plant schema. Customer defect rows reach the defect catalogue; readings reach parameter observations; genealogy keys reach edges. | Relational transform blocks |
| **S2 Widget and chart binding** | The dataset a widget displays: parameters, derived variables, grouping, the query behind a chart | Relational and aggregation blocks |
| **S3 Analysis authoring** | Correlation, statistics and mathematical analysis. Example: relate one hundred parameters in one correlation block, save it, chart the result | Statistical and correlation method blocks |
...
Neither mode is a lesser citizen. Both produce the same artifact class: a named, versioned, saved definition. Neither is hidden behind a licence tier that the other is not.

### 6.3 Mode one: block and wiring diagram

**Right panel, the block palette.** Grouped, searchable, dragged onto the board. Groups include, and are extensible by registry entry and never by code branch:

- **Arithmetic and logical**: addition, subtraction, comparison, `AND`, `OR`, `NOT`
- **Statistics, correlation and analysis functions**: the method toolbox
- **Control and conditional**: `if / else`, `where`, and iteration constructs where the surface's semantics admit them, authored as approachable nested blocks in the manner of a visual teaching language rather than as text
- Further groups per surface purpose

**Left panel, the schema browser.** A three-level tree, each level unfolding:
...
For S1 only, the left panel presents **two** schema groups, the staging shapes and the plant schema, because moving data between them is S1's entire purpose.

**The left panel shows only what the customer owns.** Emulated-source schemas, staging residue and platform infrastructure are never listed. A schema named after an emulated source appearing in a customer's tree is a Rule 1 violation visible to the customer.

**Centre, the board.** The user drags parameters from the left, drags blocks from the right, wires them, presses **Run**, and on a correct result presses **Save as a named file**.

**Bottom, the debug log.** Always present, three severities, each carrying a written description:
...
The log is the surface through which a non-programmer learns the tool. A red outline with no sentence beside it is a failure of this specification, not a minor styling gap.
...
3. **Bottom debug log unchanged.** Error, warning and success, each with a description, now describing the SQL.
4. **The board becomes a SQL editor.** The user writes ordinary SQL.
5. **Run.** Returned rows appear below. On a correct result, **Save as a named file**.

### 6.5 Board semantics follow purpose, because the join happens once

*This clause resolves what appeared to be a contradiction between the palette specification and the layer-separation principle. It is not a compromise; it follows from a property of the journey.*

**The join is declared once, in S1, and never again.**

Journey step 4 has the user joining piece identity from one source with material identity from another. The reason is stated in Rule 1's rationale: **each customer knows his own plant, and the vendor cannot know the schema architecture of every customer's plant.** Establishing those relationships is not a step repeated inside every authoring act. It is the purpose of the first surface.

Therefore:

| Surface | What flows on the board | Consequence |
|---|---|---|
| **S1** | **Datasets.** Multiple source tables, joins between them, filters, mappings into the plant schema. Relationships between tables are created here. | The board is a dataflow graph and a wire carries rows. Arithmetic and comparison are configuration inside a node, because two unrelated tables have no row correspondence until a join declares one. |
| **S2 to S5** | **One prepared dataset.** The relationships already exist, declared upstream in S1. There is a single row context. | A wire may carry a value. Comparison and arithmetic blocks sit on the board directly, because "this column against that column" now has exactly one meaning: within the same row of the prepared model. |

This is one shell whose board semantics follow purpose, exactly as its palette and left panel already do.

**Control flow remains orchestration.** A saved definition describes what its output is. How often it runs, over what window, and in what sequence relative to other definitions belongs to the job that carries it, not to the transform graph.

### 6.6 Illegal wiring is refused at drag time
...
**On S1**, a wire is illegal when: a dataset is connected to a value input, or the reverse; a required input is unconnected; the graph contains a cycle; or **two tables are compared with no join declared between them.** That last case is the mistake a plant engineer will actually make, and the message must name it.

**On S2 to S5**, a wire is illegal when: a column type does not accept the operation, such as text into arithmetic; a referenced column is not present in the prepared dataset; or an aggregate is used outside an aggregation context.

Both sets are enumerable, both are checkable at drag time, and both produce a written sentence in the debug log of 6.3.

**Measured status, 25 July 2026.** The shipped canvas implements its connection handler as an unconditional edge addition with no validity check. **Every wire between every pair of ports is currently accepted.** The port colours are decoration, not enforcement. This clause is written and not implemented, and closing that gap is the highest-value single fix in the authoring layer.

### 6.7 Widget authoring uses this shell, and there is no second door

**This resolves how a user selects the dataset a widget displays.**
...
In every case the shell opens with the widget's **current definition already loaded**, so the user edits what is there rather than starting from nothing. They may modify the existing SQL, write new SQL, or rewire the diagram. On save, the widget's definition is versioned like every other definition.

**There is no separate widget-binding mechanism and no second way to select a dataset.** This shell is the only door for widget data, exactly as the DB-link is the only door for plant data.

**Measured status, 25 July 2026.** No such surface exists. The only binding mechanism in the product is a plain text area accepting a key-value script, gated behind a higher licence tier and reachable from no control. The widget card's Edit action shares its handler with Rename. This clause is written and not implemented.

### 6.8 Milestone placement

Stated so this chapter is not read as a short-term promise.

| Clause | Status | Milestone |
|---|---|---|
| 6.6 wiring legality on the existing S1 canvas | Not implemented; small and high value | **M1** |
| 6.3 left panel three-level unfolding tree | Partially present in S1 | M2 |
| 6.3 debug log with described severities | Not implemented as specified | M2 |
| 6.4 SQL mode on any surface | Not implemented anywhere | M2 |
| 6.7 widget authoring through the shell | Not implemented | M2 |
| 6.1 one shell serving all five purposes | Not implemented; the five surfaces currently differ | M2 |
| 6.2 the mode toggle | Not implemented | M2 |

S1 is the surface to demonstrate today, because it is the only one carrying a real node canvas. The remaining clauses are stated as pilot roadmap in one sentence, per Rule 3.
## 7. The Engine
...
Part II.6.5, board semantics follow purpose, and Part II.6.6, illegal wiring is refused at drag time, are preconditions for everything below. This specification details the S1 case, where a wire carries a dataset.
...
Part II.6.2 requires both modes on every surface. This clause states exactly what "switching" does, because a naive claim of lossless round-tripping between a visual graph and arbitrary SQL is a reliability trap. The contract is explicit and is stated to the user in the interface, not buried here.

- **Visual to SQL is always available and deterministic.** The graph compiles to SQL for the target dialect and is viewable at any time. This is not a mode switch; it is a view, and it never alters the definition.
- **SQL is a first-class authoring mode**, not a transient view. The user writes SQL directly; it is validated by parse, schema inference and a sampled dry run, and it produces the same artifact type with the same versioning.
...
- **SQL to visual reconstruction is best-effort and bounded.** It is offered only for the subset the parser can faithfully map: simple select, filter, join, group and sort. For window functions, vendor-specific syntax, correlated subqueries or common table expressions the tool keeps the definition in SQL mode rather than producing a wrong or partial graph, and says so in the debug log.

**The product does not pretend a full round trip exists.** Claiming one and then losing a user's work is a worse failure than stating the limit honestly at the moment of the switch.
### 14.5 Node catalogue
...
| `rules.txt` Rule 6 | Four low-code surfaces | Reconciled with the later five-surface passage; both were correct at different granularities. Rewritten in full in v3.0 as Part II.6, the shared shell. |
...
| The v2.0 claim that arithmetic and flow-control blocks never belong on any board | Corrected in v3.0 by Part II.6.5. They do not belong on S1's board, where a wire carries a dataset. They do belong on S2 to S5, where one prepared dataset gives a single row context. Control flow remains orchestration on every surface. |
...
| Part II.6, the shared authoring shell in full | v3.0. Author's clarification of the M1 rule, 25-Jul |
| Part II.6.5, board semantics follow purpose because the join happens once | v3.0. Derived from journey step 4 and the generic-only rationale: the vendor cannot know each customer's schema architecture |
| Part II.6.6, illegal wiring refused at drag time, enumerated per surface | v3.0. Measured defect: the shipped canvas accepts every connection |
| Part II.6.7, widget authoring through the shell, no second door | v3.0. Closes a question the product had never answered |
| Part III.14.4 rewritten | v3.0. Aligns the dual-mode contract with the mode toggle |
