...
/// timeout. User values are ALWAYS parameterized; identifiers are strictly whitelisted; the
/// emitted SQL is re-checked by SafeSqlValidator (table-approval is enforced by the runtime
/// resolver, so approval-only notes are not treated as compile failures).
/// </summary>
public static class WidgetScriptSqlCompiler
{
    ...
    public static WidgetScriptCompileResult Compile(string? script, int? requestedRowLimit = null, int? requestedTimeoutSeconds = null)
    {
        ...
        var fatal = safety.Errors.Where(IsFatalSafetyError).ToArray();
        if (fatal.Length > 0)
            return new WidgetScriptCompileResult(false, sql, parameters, rowLimit, timeout, "UnsafeSql",
                "Compiled SQL failed SafeSqlValidator: " + string.Join("; ", fatal));

        return new WidgetScriptCompileResult(true, sql, parameters, rowLimit, timeout, null, null);
    }

    // Approval-only notes (e.g. "table X is not in the allowed list") are enforced by the runtime
    // resolver, not the compiler. Only genuinely unsafe constructs are fatal at compile time.
    private static bool IsFatalSafetyError(string error)
    {
        var e = error.ToLowerInvariant();
        return e.Contains("forbidden")
            || e.Contains("multiple")
            || e.Contains("statement")
            || e.Contains("only select")
            || e.Contains("only a single")
            || e.Contains("comment")
            || e.Contains("empty")
            || e.Contains("system")
            || e.Contains("information_schema")
            || e.Contains("pg_")
            || e.Contains("recursive");
    }
