...
public sealed class CorrelationReaperSourceGuardTests
{
    ...
    public void Learning_function_no_longer_writes_phantom_columns_or_swallows_errors()
    {
        ...
        // The phantom columns are ASSIGNMENT TARGETS on the compute-runs table. Assert the
        // defect form (column = ...), not the bare token, so a legitimate read of the
        // v_result_count VARIABLE inside the message string does not false-positive.
        Assert.DoesNotContain("finished_at_utc =", match.Value);
        Assert.DoesNotMatch(new Regex(@"[^_]result_count\s*="), match.Value);
