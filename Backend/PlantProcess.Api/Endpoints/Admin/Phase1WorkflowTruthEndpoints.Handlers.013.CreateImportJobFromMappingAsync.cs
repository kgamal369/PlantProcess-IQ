...
private static async Task<IResult> CreateImportJobFromMappingAsync(
...

    {
...

        {
            ...
            PlantProcess.Application.Jobs.Admission.JobLaneAssignment.Initialize(existing);
...

        {
            ...
            var poolError = PlantProcess.Application.Jobs.Admission.JobLaneAssignment.ValidateChange(existing, JobDefinitionType.CanonicalRefresh);
            if (poolError is not null) return Results.BadRequest(new { error = poolError.Message });
            ...
            PlantProcess.Application.Jobs.Admission.JobLaneAssignment.CompleteFamilyChange(existing);
