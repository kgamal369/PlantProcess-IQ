...
if (!builder.Services.Any(service => service.ServiceType == typeof(FirstRunProvisioningState)))

{

    builder.Services.AddSingleton<FirstRunProvisioningState>();

}



if (!builder.Services.Any(service => service.ServiceType == typeof(AuthStore)))
...
if (!builder.Services.Any(service => service.ServiceType == typeof(IPlantEntitlementResolver)))

{

    builder.Services.AddScoped<IPlantEntitlementResolver, PlantEntitlementResolver>();

}

P01P02StartupGuard.Validate(builder.Configuration, builder.Environment);
    ...
    PlantProcess.Analytics.Engine.Postgres.PostgresEngineServiceCollectionExtensions.AddManagedStatisticalEngineWithPostgres(builder.Services);
...
// PPIQ-T003: single intentional assignment.
    ...
    // PPIQ FIX: resolve the JWT signing key (and issuer/audience) per-request from the

    // same AuthOptions the token minter uses. Program.cs captured the validation key at

    // startup BEFORE WebApplicationFactory layered its in-memory test config, so validate

    // used appsettings (52 chars) while mint used the injected 40-char test key -> every

    // signature failed -> 401 in integration tests. Resolving per-request keeps mint and

    // validate identical in every host (Kestrel, WebApplicationFactory, production).

    builder.Services

        .AddOptions<JwtBearerOptions>(JwtBearerDefaults.AuthenticationScheme)

        .Configure<IOptionsMonitor<AuthOptions>>((jwt, authMonitor) =>
...
builder.Services.AddScoped<IPrivateModelGatewayTransport, NoopPrivateModelGatewayTransport>();
builder.Services.AddScoped<PrivateModelGatewayService>();
builder.Services.AddScoped<ConnectorBehaviourCertificationService>();
...
app.MapPhase10LicenseEndpoints();

    app.MapJobAdminEndpoints();

    app.MapConnectorAdminEndpoints();
app.MapConnectorSchemaDriftEndpoints();
...
// PPIQ-T05: diagnostic proof/certification endpoints are gated OUT of Production.
// They register only in Development or when PPIQ_DIAGNOSTIC_ENDPOINTS=true.
// MapP15HonestyCertificationEndpoints is intentionally NOT here: it is a customer-facing
// trust surface and stays registered in Production (registered separately above).
if (app.Environment.IsDevelopment() ||
    string.Equals(app.Configuration["PPIQ_DIAGNOSTIC_ENDPOINTS"], "true", StringComparison.OrdinalIgnoreCase))
{
