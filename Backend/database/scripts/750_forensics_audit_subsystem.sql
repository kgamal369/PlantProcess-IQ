...
--
-- PRODUCT FIX per T-006: a schema object that exists only as data belongs in
-- a numbered migration. Generated from the live catalog. Nothing retyped.
--
-- IDEMPOTENT AND NON-DESTRUCTIVE. At a customer this schema holds real audit
-- history, so nothing here drops a table or a schema.
...
-- ---- constraints ----------------------------------------------------
DO $mig$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ppiq_catalog_audit_pkey') THEN ALTER TABLE public.ppiq_catalog_audit ADD CONSTRAINT ppiq_catalog_audit_pkey PRIMARY KEY (id); END IF; END $mig$;
DO $mig$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ppiq_purge_audit_pkey') THEN ALTER TABLE public.ppiq_purge_audit ADD CONSTRAINT ppiq_purge_audit_pkey PRIMARY KEY (id); END IF; END $mig$;
DO $mig$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'wipe_audit_pkey') THEN ALTER TABLE ppiq_forensics.wipe_audit ADD CONSTRAINT wipe_audit_pkey PRIMARY KEY (id); END IF; END $mig$;
