<#
====================================================================================================
 PlantProcess IQ — Docker image audit + safe cleanup
====================================================================================================
...
& {
  ...
  # The KNOWN orphan set: old build-name schemes, an unused engine tag, a redundant moving tag.
  # (Everything else is left strictly alone.)
  $orphanCandidates = @(
    'plantprocessiq-plantprocess-website:latest',   # old compose-project build of website
    'plantprocessiq-plantprocess-workers:latest',   # old compose-project build of workers
    'plantprocessiq/app-web:local',                 # older naming scheme
    'plantprocessiq/website:local',                 # older naming scheme
    'postgres:17-alpine',                           # stack runs postgres:16-alpine
    'mysql:8.4'                                     # same image ID as mysql:8 (drops the extra tag only)
  )
