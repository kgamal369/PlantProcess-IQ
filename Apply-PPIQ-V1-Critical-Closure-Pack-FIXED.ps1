...
<#
.SYNOPSIS
  PlantProcess IQ V1 critical closure pack v2 for:
...
if ($SmokePassword -match '^(YOUR_REAL_E2E_PASSWORD|CHANGE_ME|CHANGEME)$') {
    throw "PPIQ_SMOKE_PASSWORD still contains a placeholder. Set the real e2eadmin password."
}
...
if (-not $SkipHttpsMatrix -and ($AppHttpsUrl -match 'YOUR[-_]' -or $WebsiteHttpsUrl -match 'YOUR[-_]')) {
    throw "PPIQ HTTPS URLs still contain placeholders. Set real HTTPS addresses or use -SkipHttpsMatrix."
}
...
# test component because TypeScript infers a void return. Use ASCII-only, single-quoted anchors here
# for reliable parsing under Windows PowerShell 5.1.
...
$boomUntypedAnchor = 'function Boom() {'
$boomNeverAnchor = 'function Boom(): never {'
$boomReactElementAnchor = 'function Boom(): ReactElement {'
$boomQualifiedReactElementAnchor = 'function Boom(): React.ReactElement {'

if ($boomText.IndexOf($boomUntypedAnchor, [System.StringComparison]::Ordinal) -ge 0) {
    Replace-Required `
        -Path $boomTest `
        -Old $boomUntypedAnchor `
        -New $boomNeverAnchor `
        -Description 'Repair TS2786 throw-only test component'
}
elseif (
    $boomText.IndexOf($boomNeverAnchor, [System.StringComparison]::Ordinal) -lt 0 -and
    $boomText.IndexOf($boomReactElementAnchor, [System.StringComparison]::Ordinal) -lt 0 -and
    $boomText.IndexOf($boomQualifiedReactElementAnchor, [System.StringComparison]::Ordinal) -lt 0
) {
