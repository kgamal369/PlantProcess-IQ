...
for (const file of files) {
  ...
  lines.forEach((line, index) => {
    if (/<(table|thead|tbody|tfoot|tr|td|th)\b/.test(line)) {
      findings.push({
        file: rel,
        line: index + 1,
        kind: "raw-table-markup",
        message: "raw table markup is not allowed outside canonical standard table wrappers",
...
const md = [
  ...
  "",
  findings.length === 0
    ? "## Result\n\nGREEN — raw table markup outside standard wrappers is clean, SortableDataTable delegates to StandardDataTable, and table state/accessibility contract is present."
