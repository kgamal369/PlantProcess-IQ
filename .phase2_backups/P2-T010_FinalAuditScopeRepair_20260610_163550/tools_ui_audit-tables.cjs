...
function hasStandardTableWrapper(text) {
  return (
    /<StandardP2Table\b/.test(text) ||
    /<StandardDataTable\b/.test(text) ||
    /<SortableDataTable\b/.test(text)
  );
}
...
for (const file of files) {
  ...
  const wrapped = hasStandardTableWrapper(text);
  ...
  lines.forEach((line, index) => {
    ...
    if (!wrapped && /<(thead|tbody|tfoot|tr|td|th)\b/.test(line)) {
      findings.push({
        file: rel,
        line: index + 1,
        kind: "unwrapped-table-markup",
        message: "table row/cell markup must be inside StandardP2Table, StandardDataTable, or SortableDataTable",
        snippet: line.trim().slice(0, 220),
      });
    }
  });
...
const md = [
  ...
  "",
  findings.length === 0
    ? "## Result\n\nGREEN — raw table roots are clean, table row/cell markup is wrapped by standard table components, SortableDataTable delegates to StandardDataTable, and table state/accessibility contract is present."
