...
export const getAnalysisReadiness = (outcomeKey: string, grain = "coil", windowDays = 3650) =>
  apiClient.get<AnalysisReadinessDto>(`${BASE}/readiness?${qs({ outcomeKey, grain, windowDays })}`);

export const getAnalysisReadinessGates = (outcomeKey: string, grain = "coil", windowDays = 3650) =>
  apiClient.get<AdvancedReadinessGateSummaryDto>(`${BASE}/readiness/gates?${qs({ outcomeKey, grain, windowDays })}`);
export const runCorrelation = (outcomeKey: string, grain = "coil", windowDays = 3650) =>
